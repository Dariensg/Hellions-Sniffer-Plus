...
public class StonePineBoatItem extends Item {
    private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
    private final boolean hasChest;

    public StonePineBoatItem(boolean bl, Item.Properties properties) {
        super(properties);
        this.hasChest = bl;
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        if (((HitResult)hitResult).getType() == HitResult.Type.BLOCK) {
            StonePineBoat boat = this.getBoat(level, hitResult);
    ...
    private StonePineBoat getBoat(Level level, HitResult hitResult) {
        if (this.hasChest) {
            return new StonePineChestBoat(level, hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z);
        }
        return new StonePineBoat(level, hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z);
    }
}
